...
import {
  CreateJewelryMock,
  JewelryRepositoryMock,
  ListJewelrysMock,
  UpdateJewelryMock,
  UpdatedJewelryMock,
} from '../testing/jewelry/index';
...
describe('JewelryService', () => {
  ...
  describe('FindAllJewelry', () => {
    ...
    it('should return a message of jewelrys register have no one jewelry', async () => {
      jest
        .spyOn(JewelryRepositoryMock.useValue, 'find')
  ...
  describe('Give a jewel to users', () => {
    it('Should be possible to give a jewel to users', async () => {
      const userWithJewelry = {
        ...ListUsersMock[0],
        jewelries: [{ type: ListJewelrysMock[0].type }],
      };
      jest
        .spyOn(UserRepositoryMock.useValue, 'findOne')
        .mockResolvedValue(userWithJewelry as never);

      jest
        .spyOn(JewelryRepositoryMock.useValue, 'findOne')
        .mockResolvedValue(ListJewelrysMock[0] as never);

      const result = await service.GiveJewelryToUser(
        ListUsersMock[0].id,
        ListJewelrysMock[0].id,
      );
    ...
    it('Should be possible to give a new jewel to users', async () => {
      ...
      jest
        .spyOn(JewelryRepositoryMock.useValue, 'findOne')
        .mockResolvedValue(ListJewelrysMock[1] as never);

      const result = await service.GiveJewelryToUser(
        ListUsersMock[0].id,
        ListJewelrysMock[1].id,
      );
    ...
    it('Should return an error when no user is found', async () => {
      ...
      const result = service.GiveJewelryToUser(
        ListUsersMock[0].id,
        ListJewelrysMock[0].id,
      );
  ...
  describe('Soft delete', () => {
    it('Should be possible remove a jewelry from database', async () => {
      ...
      expect(result).toEqual({
        result: `Jewelry with id ${ListJewelrysMock[0].id} has been remove.`,
      });
    });
